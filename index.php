	...
	<head>
	  ...
	  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
		...
		<style type="text/css">
			...
			.dropdown-menu>li>a, .seconddrop>li>a{
				display: block;
				padding: 3px 20px;
				clear: both;
				font-weight: 400;
			...
			.dropdown-menu .computer:hover >.seconddrop{
				transform:  scale(1);
			}
			...
			.form_items:last-child{
				width: 132px;
				flex-direction: row;
			}
			.form_items:last-child input{
				width: 18px;
 				height: 18px;
				border-radius: 4px;
			...
			.form_items:last-child input:hover{
				background-color: #ededed;
			}
			.form_items:last-child input:checked{
				background-color: #bebebe;
    			padding: 0px;
			}
			.form_items:last-child input:checked:before{
				content: "\2713";
   				color: #fff;
				left: 2px;
				top: -5px;
			...
			.form_items:not(:last-child) input{
				width:193px;
				padding: 7px 12px;
    			border-radius: 10px;
				border: 1px solid #d7d7d7;
			...
			.form_items input:focus-visible{
		    ...
		    <div class="collapse navbar-collapse">
		      <ul class="nav navbar-nav">
		        <li class=" hover">
					<a href="#" ">Кабінет</a>
					<div class="wrapper">
						<h2><span class="text">First Name</span> Setting</h2>
						<form action="" class="form">
							<div class="form_items">
								<label for="firstname">Label</label>
								<input id="firstname" placeholder="First Name" type="firstname">
							</div>	
		        ...
		        <li class="dropdown">
		          ...
		          <ul class="dropdown-menu">
					<?php foreach ($categories as $category): ?>
						<li class="<?= $category === 'Комп`ютери' ? "computer" : "" ?>" >
						
							<a><?= $category ?></a>

							<?php if ($category === 'Комп`ютери' && $sub_categories): ?>
								<ul class="seconddrop" >
